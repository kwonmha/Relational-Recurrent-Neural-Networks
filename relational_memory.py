...
class RelationalMemory(rnn_core.RNNCore):
    ...
    def _multihead_attention(self, memory):
        ...
        print(memory.get_shape(), "memory")  #(5, 7, 64)
        print(qkv.get_shape(), "qkv")  #(5, 7, 192)
        ...
        print(qkv_transpose.get_shape(), "qkv_transpose")
    ...
    def _create_gates(self, inputs, memory):
        ...
        memory = tf.tanh(memory) # (b, slot, mem_size)
        inputs = basic.BatchFlatten()(inputs)
        # print(inputs.get_shape(), "inputs") (b, unit_size)
        gate_inputs = basic.BatchApply(basic.Linear(num_gates), n_dims=1)(inputs) # n_dims=1 as input is vector # (b, 2 * mem_size or 1)
        gate_inputs = tf.expand_dims(gate_inputs, axis=1)  # (b, 1, mem_size or 1)
        gate_memory = basic.BatchApply(basic.Linear(num_gates))(memory)  # (b, slot, 2 * mem_size or 1)
        # gate = gate_memory + gate_inputs  #(b, slot, mem_size * 2) broadcasting
        gates = tf.split(gate_memory + gate_inputs, num_or_size_splits=2, axis=2)
        input_gate, forget_gate = gates
        input_gate = tf.sigmoid(input_gate + self._input_bias)  #(b, slot, mem_size('unit') or 1('memory'))
    ...
    def _build(self, inputs, memory, treat_input_as_matrix=False):
        ...
        """
        print(inputs.get_shape())  #(5, 3, 3)
        print(memory.get_shape())  #(5, 4, 64)
        if treat_input_as_matrix:
            ...
            print(inputs_reshape.get_shape())  #(5, 3, 64)
        else:
            inputs = basic.BatchFlatten()(inputs)
        ...
        # (5, 7, 64) tensorflow.python.framework.tensor_shape.Dimension' 'int'
        # print(type(memory_plus_input.get_shape()[0]), type(memory_plus_input.get_shape().as_list()[0]))
        next_memory = self._attend_over_memory(memory_plus_input)  #(b, slot+input_length, mem_size)
        print(next_memory.get_shape(), "next_memory before slicing")
        ...
        if self._gate_style == 'unit' or self._gate_style == 'memory':
            ...
            print(inputs_reshape.get_shape(), "inputs_reshape")
            print(next_memory.get_shape(), "next_memory")
            print(self._input_gate.get_shape(), "input_gate")
